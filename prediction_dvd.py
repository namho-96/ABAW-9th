...
# 1. 입력 H5 파일 경로
H5_FILE_PATH = "C:/path/to/your/test_data.hdf5"

# 2. 학습된 모델(.pth) 파일 경로
MODEL_PATH = "C:/path/to/your/best_model_fold1_image_text_video.pth"

# 3. 템플릿 CSV 파일들이 있는 폴더 경로
TEMPLATE_DIR = "C:/path/to/your/template_csv_folder"

# 4. 최종 예측 결과를 저장할 폴더 경로
OUTPUT_DIR = "./predictions_output"

# 5. 모델이 학습된 모달리티 조합 (학습 시와 동일하게)
MODALITIES = "video_text_visual" # 예: "video_text_visual" 또는 "image_video"

# 6. 예측에 사용할 장치 ('cuda' 또는 'cpu')
DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu'

# ==================================================================
#  Modellefinition (Unverändert aus train_total_v2.py)
# ==================================================================
...
class UnifiedClassifier(nn.Module):
    """유연한 멀티모달 분류기 - train_total_v2.py와 동일한 구조"""
    ...
    def __init__(self, 
        ...
        if 'image' in modalities_internal:
            self.encoders['image'] = SequenceEncoder(feat_dims.get('visual', feat_dims.get('image')), enc_hidden_dim, dropout=dropout)
        if 'text' in modalities_internal:
            self.encoders['text'] = SequenceEncoder(feat_dims['text'], enc_hidden_dim, dropout=dropout)
        if 'video' in modalities_internal:
        ...
        if len(self.modalities) > 1:
            if self.image_centric:
                ...
                self.cross_attention = nn.ModuleDict({
                    ...
                    for m in modalities_internal
                })
                self.layer_norms = nn.ModuleDict({
                    m: nn.LayerNorm(self.model_dim) for m in modalities_internal
                })
        ...
        if self.task == 'violence':
            ...
            raise ValueError(f"Task '{self.task}' is not supported for prediction in this script.")
...
def main():
    """테스트 데이터셋에 대한 예측을 수행하고 CSV 파일로 저장합니다."""
    print("🚀 Starting prediction with hardcoded settings.")
    ...
    try:
        with h5py.File(H5_FILE_PATH, 'r') as h5f:
            ...
            video_names = [name.decode('utf-8') for name in video_names_bytes]
            
            feat_dims = {
                'video': video_feats.shape[-1],
                'text': text_feats.shape[-1],
                'visual': visual_feats.shape[-1]
            }
    ...
    with torch.no_grad():
        num_segments = len(video_names)
        for i in tqdm(range(num_segments), desc="Predicting segments"):
            features = {}
            if 'video' in modalities_list:
                features['video'] = torch.from_numpy(video_feats[i]).unsqueeze(0).float().to(DEVICE)
            if 'text' in modalities_list:
                features['text'] = torch.from_numpy(text_feats[i]).unsqueeze(0).float().to(DEVICE)
            if 'visual' in modalities_list:
                features['visual'] = torch.from_numpy(visual_feats[i]).unsqueeze(0).float().to(DEVICE)

            outputs = model(**features)
            pred_label = torch.argmax(outputs, dim=1).item()
            video_predictions[video_names[i]].append((segment_ids[i], pred_label))

    print("\n[Step 4/4] Generating prediction CSV files...")
    unique_video_names = sorted(list(video_predictions.keys()))

    for video_name in tqdm(unique_video_names, desc="Writing CSV files"):
        template_path = os.path.join(TEMPLATE_DIR, f"{video_name}.csv")
        output_path = os.path.join(OUTPUT_DIR, f"{video_name}.csv")

        if not os.path.exists(template_path):
            print(f"Warning: Template file not found for {video_name}, skipping.")
            continue

        df = pd.read_csv(template_path)
        segment_preds = sorted(video_predictions[video_name], key=lambda x: x[0])
        
        if not segment_preds:
            print(f"Warning: No predictions found for video {video_name}. Filling with 0.")
            df['Label'] = 0
            df.to_csv(output_path, index=False)
            continue
            
        pred_map = {seg_id: label for seg_id, label in segment_preds}
        last_known_label = segment_preds[0][1]
        
        labels_to_fill = []
        for frame_num in df['Frame_Num']:
            target_segment_id = (frame_num - 1) // 64
            
            if target_segment_id in pred_map:
                current_label = pred_map[target_segment_id]
                last_known_label = current_label
            else:
                current_label = last_known_label
            
            labels_to_fill.append(current_label)
            
        df['Label'] = labels_to_fill
        df.to_csv(output_path, index=False)

    print(f"\n✅ Prediction complete! Results are saved in '{OUTPUT_DIR}'.")
